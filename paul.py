...
class Paul(object):
	def __init__(self):
		self.headers = {"User-Agent": "paul/%s" % str(__version__), "Content-type": "application/x-www-form-urlencoded", "Accept": "text/xml"}
		self.prowlapi = str(__prowlapi__)

	"""
	apikey -> API keys separated by commas. Each API key is a 40-byte hexadecimal string
	...
	"""
	def push(self, apikey, application, event, description, url=None, priority=None, providerkey=None):
		data = dict()
		...
		if(response.status == 200):
			...
			raise Exception("Not authorized, the API key (%s) given is not valid, and does not correspond to a user", apikey)
			...
			raise Exception("Internal server error, something failed to execute properly on the Prowl side")
